...
| 2026-07-22 | Phase 8.1 complete: merged main, added CI, redeployed to Cloud Run | Post-hackathon restructure hygiene closed. PR #2 (worktree backlog + rate limit + token metering + logging fix) and PR #1 (tracked docs + CLAUDE.md correction) merged to main. CI (`.github/workflows/ci.yml`) gates every PR on pytest/tsc/build and was green (658 passed) before deploy. Redeployed same two Cloud Run services to new revisions `sadify-api-00010-m9c` and `sadify-web-00005-499` from Cloud Shell. Smoke: /health ok, live guest Q&A, `gemini_token_usage` visible in Cloud Logging, zero 5xx. Deployment is now gated on green CI, not on a checkpoint. | CLAUDE.md, CURRENT, TC-027, PR #1, PR #2 |
